...
data{
  ...
  int age0; // cut off at which point FoI changes
...
model{
  for(a in 1:A){
    target+= cat2_varFoI_lpmf(pos[a]|N[a], foi1, foi2, sigma, age0, age1[a], age2[a]);
  }
}
...
generated quantities{
  ...
  for(a in 1:A){
   log_lik_a[a] = cat2_varFoI_lpmf(pos[a]|N[a], foi1, foi2, sigma, age0, age1[a], age2[a]);
   if(age2[a] <= age0){
        seroprevalence[a] = p_seroprev_2(age1[a], age2[a], foi1, sigma);
   } else{
     seroprevalence[a] = p_seroprev_2_varFoI(age1[a], age2[a], age0, foi1, foi2, sigma);
   }
